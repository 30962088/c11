...
public class BBSCommentListAdapter extends BaseAdapter implements PinnedSectionListAdapter {
	...
	public BBSCommentListAdapter(Context context, List<Object> list,OnCommentBtnClickListener onCommentBtnClickListener) {
		super();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(obj instanceof BBSDetailHeaderView.Model){
			BBSDetailHeaderView headerView = new BBSDetailHeaderView(context);
			headerView.setModel((BBSDetailHeaderView.Model)obj);
			...
			ImageLoader.getInstance().displayImage(model.getAvatar(), holder.avatar,DisplayOptions.IMG.getOptions());
		...
		new IOSPopupWindow(context, new IOSPopupWindow.Params("请输入举报理由", new ArrayList<String>(){{
			...
			public void oniositemclick(int pos, String text) {
				InsertcommentreportRequest request = new InsertcommentreportRequest(context,
						new InsertcommentreportRequest.Params(text, model.getId(), model.getReportType(), APP.getSession().getSid(), APP.getSession().getPkey()));
